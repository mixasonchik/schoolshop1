...
function updateStatus(){
  ...
  document.getElementById('timeText').innerHTML = open
    ? 'Предзаказы открыты до 11:20.'
    : 'Заказы на сегодня закончились. Откроемся завтра в <b>8:50</b>.';
}

function productImage(p, cls = ''){
  return `<div class="product-img ${p.white ? 'white-bg' : ''} ${cls}">
    ...
    <div class="mini-card">
      ...
      <div class="mini-body">
        <div class="mini-price">
          ${p.price ? p.price + ' ₽' + (p.old ? ' <span class="mini-old">' + p.old + ' ₽</span>' : '') : 'Цена позже'}
        </div>
...
function renderCart(){
  ...
  if(!items.length){
    ...
    list.innerHTML = items.map(([id, qty]) => {
      ...
      return `<div class="cart-item">
        <div class="cart-thumb ${p.white ? 'white-bg' : ''}">
          <img src="${p.image}" alt="${p.name}">
        </div>
        <div class="cart-info">
          <b>${p.name}</b><br>
          <span class="muted">${p.price ? p.price + ' ₽' : 'Цена позже'} × ${qty}</span>
        </div>
        <div class="qty">
  ...
  document.querySelectorAll('[data-minus]').forEach(b => {
    b.onclick = () => {
      state.cart[b.dataset.minus]--;
      if(state.cart[b.dataset.minus] <= 0) delete state.cart[b.dataset.minus];
      save();
      renderCart();
    };
  });
  ...
  const total = items.reduce((s, [id, q]) => {
    return s + ((products.find(p => p.id === id).price || 0) * q);
  }, 0);
...
async function placeOrder(){
  ...
  const order = {
    ...
    items: items.map(([id, qty]) => ({
      product: products.find(p => p.id === id).name,
      qty
    })),
    total: items.reduce((s, [id, q]) => {
      return s + ((products.find(p => p.id === id).price || 0) * q);
    }, 0)
  };
  ...
  try {
    ...
    return toast('Ошибка связи с сервером. Попробуй позже');
  }
  ...
  state.orders.push({
    ...
    items: order.items.map(i => `${i.product} ×${i.qty}`).join(', '),
    address: `${state.floor} этаж, шкафчик ${locker}`,
    status: 'отправлен'
  });
...
document.getElementById('quickReserve').onclick = () => {
  addToCart('dubai');
